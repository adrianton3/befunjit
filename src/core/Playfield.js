...
(function() {
  'use strict';
  var Playfield, S, initField, initPathPlane, initSize;

  S = bef.Symbols;

  Playfield = function(string, options) {
    var lines, ref;
    lines = string.split('\n');
    ref = initSize(lines, options), this.width = ref.width, this.height = ref.height;
    this.field = initField(lines, this.width, this.height);
    this.pathPlane = initPathPlane(this.width, this.height);
  };

  initSize = function(lines, options) {
    var height, width;
    width = Math.max.apply(Math, lines.map(function(line) {
      return line.length;
    }));
    height = lines.length;
    if ((options != null ? options.size : void 0) === 'standard') {
      return {
        width: 80,
        height: 25
      };
    } else if ((options != null ? options.size : void 0) === 'double') {
      return {
        width: width * 2,
        height: height * 2
      };
  ...
  initField = function(lines, width, height) {
    ...
    while (i < iLimit) {
      line = lines[i];
      chars = Array.from(line, function(char) {
  ...
  Playfield.prototype.setAt = function(x, y, value) {
