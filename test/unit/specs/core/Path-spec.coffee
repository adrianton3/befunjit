describe 'Path', ->
	...
	describe 'push', ->
		it 'pushes some entries', ->
			...
			(expect path.getAsList()).toEqual [
				{ x: 10, y: 20, dir: 3, charCode: 'z', string: false }
				{ x: 1, y: 2, dir: 0, charCode: 't', string: false }
			]
	...
	describe 'prefix', ->
		...
		it 'gets the prefix of an arbitrary length', ->
			...
			(expect prefixPath.getAsList()).toEqual [
				{ x: 11, y: 20, dir: 3, charCode: 'a', string: false }
				{ x: 12, y: 20, dir: 3, charCode: 'b', string: false }
			]
		...
		it 'gets the prefix of the same length as the source path', ->
			...
			(expect prefixPath.getAsList()).toEqual [
				{ x: 11, y: 20, dir: 3, charCode: 'a', string: false }
				{ x: 12, y: 20, dir: 3, charCode: 'b', string: false }
				{ x: 13, y: 20, dir: 3, charCode: 'c', string: false }
			]
